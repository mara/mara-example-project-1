...
SELECT util.create_enum('m_dim_next.LEAD_TYPE',
                        (SELECT array_agg(DISTINCT coalesce(lead_type, 'Unknown')) FROM m_tmp.deal));
...
SELECT util.create_enum('m_dim_next.AVERAGE_STOCK',
                        (SELECT array_agg(DISTINCT coalesce(average_stock, 'Unknown')) FROM m_tmp.deal));

SELECT util.create_enum('m_dim_next.BUSINESS_TYPE',
                        (SELECT array_agg(DISTINCT coalesce(business_type, 'Unknown')) FROM m_tmp.deal));
