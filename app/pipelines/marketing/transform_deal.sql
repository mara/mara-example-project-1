DROP TABLE IF EXISTS m_dim_next.deal CASCADE;
CREATE TABLE m_dim_next.deal
(

    deal_id                       TEXT                              NOT NULL PRIMARY KEY, -- The Marketing Qualified Lead id of the closed deal as PK
...
SELECT util.add_index('m_dim_next', 'deal',
                      column_names := ARRAY ['marketing_qualified_lead_fk', 'seller_fk']);

CREATE OR REPLACE FUNCTION m_tmp.constrain_deal()
    RETURNS VOID AS
$$
SELECT util.add_fk('m_dim_next', 'deal', 'm_dim_next', 'marketing_qualified_lead');
SELECT util.add_fk('m_dim_next', 'deal', 'ec_dim', 'seller');
$$
    LANGUAGE sql;
